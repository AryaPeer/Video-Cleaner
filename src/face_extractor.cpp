...
bool FaceExtractor::extractFaces(const std::string& videoPath, float timeInSeconds, const std::string& outputDir) {
    ...
    // Validate timestamp
    if (fps == 0) { // Avoid division by zero and handle invalid video
    ...
    // Read frame at the specified position
    cv::Mat frame;
    ...
    video.release(); // Release video capture after reading the frame
    ...
    if (faces.empty()) {
        ...
        return true; // Not an error if no faces are found
    }
    ...
    for (size_t i = 0; i < faces.size(); i++) {
        ...
        if (faceRect.width <= 0 || faceRect.height <= 0) continue; // Skip invalid ROIs
        ...
        if (!cv::imwrite(outputPath.string(), faceROI)) {
            ...
            // Optionally continue to try and save other faces
        } else {
...
bool FaceExtractor::extractFacesFromRange(const std::string& videoPath, float startTime, float endTime, 
    ...
    // Open video file to check duration and basic properties first
    ...
    if (startTime >= endTime && !(startTime == endTime && startTime == 0 && duration == 0) ) { // Allow 0 to 0 for single frame if duration is 0
         if (startTime == endTime && duration > 0) { // allow processing single frame if start == end
             // This is fine, will process one frame at startTime
         } else {
    ...
    // Extract faces at regular intervals
    ...
    for (float time = startTime; time <= endTime; time += interval) {
        ...
        if (!extractFaces(videoPath, time, outputDir)) {
            all_successful = false; // Log error but continue processing other timestamps in range if desired
            std::cerr << "Failed to extract faces at timestamp " << time << "s. Continuing with next interval." << std::endl;
        }
        if (time == endTime) break; // Ensure endTime is processed and loop terminates if interval makes time > endTime
        if (time + interval > endTime && time < endTime) { // Process endTime if it's not hit by an exact interval step
